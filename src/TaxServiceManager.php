...
/**
 * Provides the Tax Service plugin manager.
 */
class TaxServiceManager extends DefaultPluginManager implements CategorizingPluginManagerInterface {
  ...
  /**
   * Constructs a new TaxServiceManager object.
   *
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, EntityTypeManagerInterface $entity_type_manager) {
    parent::__construct(
      'Plugin/Commerce/TaxService',
      $namespaces,
      $module_handler,
      'Drupal\commerce_tax_service\Plugin\Commerce\TaxService\TaxServiceInterface',
      'Drupal\commerce_tax_service\Annotation\CommerceTaxService'
    );

    $this->alterInfo('commerce_tax_service_info');
    $this->setCacheBackend($cache_backend, 'commerce_tax_service_plugins');
  ...
  public function processDefinition(&$definition, $plugin_id) {
    ...
    foreach (['id', 'label', 'target_entity_type'] as $required_property) {
      if (empty($definition[$required_property])) {
        throw new PluginException(sprintf('The tax service %s must define the %s property.', $plugin_id, $required_property));
      }
    }

    $target = $definition['target_entity_type'];
    if (!$this->entityTypeManager->getDefinition($target)) {
      throw new PluginException(sprintf('The tax service %s must reference a valid entity type, %s given.', $plugin_id, $target));
    }
